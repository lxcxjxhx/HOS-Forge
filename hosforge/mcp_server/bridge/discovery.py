...
class MCPDiscoveryEngine:
    ...
    async def auto_register_all(self) -> list[DiscoveredService]:
        ...
        for svc in services:
            try:
                ...
                logger.info("Auto-registered MCP service: %s (%s)", svc.name, svc.protocol)
    ...
    async def _discover_from_process(self) -> list[DiscoveredService]:
        ...
        for known in KNOWN_MCP_SERVICES:
            known_dict: dict[str, Any] = known
            if known_dict.get("source") != "process":
                continue

            command = known_dict["command"]
    ...
    async def _discover_from_docker(self) -> list[DiscoveredService]:
        ...
        try:
            ...
            for line in output.strip().split("\n"):
                ...
                # 安全工具镜像识别
                if any(kw in image.lower() for kw in ["security-hub", "burp", "nuclei", "nmap"]):
                    services.append(
                        DiscoveredService(
        ...
        for port, name, desc in port_services:
            try:
                proc = await asyncio.create_subprocess_exec(
                    "sh",
                    "-c",
                    f"nc -z -w 1 127.0.0.1 {port} 2>/dev/null",
