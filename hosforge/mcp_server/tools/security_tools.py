...
def _register_scan_tools(app: FastMCP) -> None:
    ...
    async def hos_nmap_scan(
        ...
        try:
            ...
            result = await tool.run(
                target, ports=ports, scan_type=scan_type, service_detection=service_detection
            )
    ...
    async def hos_semgrep_scan(
        ...
        try:
            ...
            result = await tool.run(
                ...
                languages=(
                    [lang.strip() for lang in languages.split(",") if lang.strip()] if languages else []
                ),
                severity=severity,
            )
            return result.to_dict()
    ...
    @app.tool(
        description="Nuclei vulnerability scan - template-based automated vulnerability detection"
    )
    async def hos_nuclei_scan(
        ...
        try:
            tool = NucleiTool()
            result = await tool.run(
                target, tags=[t.strip() for t in tags.split(",")], severity=severity
            )
            return result.to_dict()
        except Exception as e:
    ...
    async def hos_burp_scan(
        ...
        try:
            ...
            logger.exception("Burp scan failed: %s", e)
            return {
                "tool_name": "burp",
                "success": False,
...
def _register_knowledge_tools(app: FastMCP) -> None:
    ...
    async def hos_cve_query(
        ...
        try:
            ...
            logger.exception("CVE query failed: %s", e)
            return []
    ...
    async def hos_cwe_query(
        ...
        try:
            ...
            logger.exception("CWE query failed: %s", e)
            return []

    @app.tool(
        description="Vulnerability explanation - explain vulnerability with CVE/CWE and code context"
    )
    async def hos_vuln_explain(
        ...
        try:
            ...
            return await kb.explain_vulnerability(
                cwe_id=cwe_id, cve_id=cve_id, code_context=code_context
            )
        except Exception as e:
            logger.exception("Vulnerability explanation failed: %s", e)
            return f"Error: {str(e)}"

    @app.tool(
        description="RAG knowledge tagging - auto-classify and tag security knowledge content"
    )
    async def hos_rag_tag(
        ...
        try:
            ...
            logger.exception("RAG tagging failed: %s", e)
            return {"tags": [], "confidence": 0.0, "source": source, "error": str(e)}
...
def _register_pentest_tools(app: FastMCP) -> None:
    ...
    async def hos_pentest_run(
        ...
        try:
            ...
            logger.exception("Pentest execution failed: %s", e)
            return {
                "success": False,
                "error": f"Pentest failed: {str(e)}",
                "findings": [],
                "summary": {},
            }
    ...
    @app.tool(
        description="Security fix suggestion - generate fix code for detected vulnerabilities"
    )
    async def hos_fix_vulnerability(
        ...
        try:
            ...
            vuln = SecurityVulnerability(
                name=vuln_name, cwe_id=cwe_id, code_snippet=code_snippet, severity=Severity.HIGH
            )
            return await agent.fix(vuln)
        except Exception as e:
            logger.exception("Fix suggestion failed: %s", e)
...
def _register_report_tools(app: FastMCP) -> None:
    ...
    async def hos_report_generate(
        ...
        try:
            ...
            return f"<html><body><h1>Error</h1><p>Report generation failed: {str(e)}</p></body></html>"
...
def _register_bridge_tools(app: FastMCP) -> None:
    ...
    async def hos_mcp_discover() -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.exception("MCP discovery failed: %s", e)
            return []
    ...
    async def hos_mcp_connect(
        ...
        """连接到指定外部安全 MCP 服务。service_name: burp-mcp / mcp-security-hub"""
    ...
    async def hos_burp_bridge(
        ...
        try:
            ...
            logger.exception("Burp bridge failed: %s", e)
            return {"success": False, "error": f"Bridge failed: {str(e)}"}
    ...
    async def hos_security_hub_bridge(
        ...
        try:
            ...
            try:
                ...
                logger.exception("Security Hub bridge call failed: %s", e)
                return {"success": False, "error": str(e)}
            finally:
                await hub.disconnect()
        except Exception as e:
            logger.exception("Security Hub bridge failed: %s", e)
            return {"success": False, "error": f"Bridge failed: {str(e)}"}
...
def _register_orchestrator_tools(app: FastMCP) -> None:
    ...
    async def hos_workflow_templates() -> dict[str, Any]:
        ...
        try:
            ...
            logger.exception("List templates failed: %s", e)
    ...
    @app.tool(
        description="Parallel security scan - run multiple independent security tests concurrently"
    )
    async def hos_parallel_scan(
        ...
        try:
            ...
            logger.exception("Parallel scan failed: %s", e)
            return {
                "success": False,
