...
def check_vram_availability() -> Dict[str, Any]:
    ...
    if not torch.cuda.is_available():
        return {"available": False, "total_vram_gb": 0, "free_vram_gb": 0, "device": "cpu"}
...
def quantize_gguf(
    ...
    try:
        # 检查工具是否存在
        subprocess.run([quantize_tool, "--help"], capture_output=True, text=True, timeout=5)
    ...
    try:
        ...
        raise QuantizationError(f"GGUF 量化失败: {e.stderr}") from e
    except Exception as e:
        raise QuantizationError(f"GGUF 量化过程中发生错误: {str(e)}") from e

    print("✓ GGUF 量化完成！")
    return output_path


def quantize_awq(model_path: str, output_path: str, bits: int = 4, group_size: int = 128) -> str:
    """AWQ 4-bit 量化
    ...
    try:
        ...
        print(
            f"  配置: zero_point={config['zero_point']}, "
        ...
        raise QuantizationError(f"缺少依赖: {e.name}\n请安装: pip install autoawq") from e
    except Exception as e:
        raise QuantizationError(f"AWQ 量化失败: {str(e)}") from e


def quantize_gptq(
    model_path: str, output_path: str, bits: int = 4, group_size: int = 128, desc_act: bool = False
) -> str:
    """GPTQ 量化
    ...
    try:
        ...
        raise QuantizationError(f"缺少依赖: {e.name}\n请安装: pip install auto-gptq") from e
    except Exception as e:
        raise QuantizationError(f"GPTQ 量化失败: {str(e)}") from e


def evaluate_perplexity(
    model_path: str, dataset: str = "wikitext", max_samples: int = 100, stride: int = 512
) -> float:
    ...
    try:
        ...
        # 计算 PPL
        print("计算 PPL...")
        metrics_result = engine.compute_metrics(predictions=references, references=references)
        ...
        raise QuantizationError(f"缺少依赖: {e.name}\n请安装: pip install datasets evaluate") from e
    except Exception as e:
        raise QuantizationError(f"PPL 评估失败: {str(e)}") from e


def convert_format(
    model_path: str, output_path: str, from_format: str, to_format: str, **kwargs
) -> str:
    """量化格式转换工具
    ...
    try:
        ...
        raise QuantizationError(f"格式转换失败: {str(e)}") from e
...
示例:
  ...
  python quantize.py --method convert --from hf --to awq --model ./model --output ./model-awq
    ...
    parser.add_argument("--output", type=str, default=None, help="输出路径（默认自动生成）")
    parser.add_argument("--bits", type=int, default=4, choices=[4, 8], help="量化位数（默认: 4）")
    parser.add_argument(
        "--quant-type", type=str, default="Q4_K_M", help="GGUF 量化类型（默认: Q4_K_M）"
    )
    parser.add_argument("--group-size", type=int, default=128, help="量化分组大小（默认: 128）")
    parser.add_argument(
        "--llama-cpp-path", type=str, default=None, help="llama.cpp 安装路径（用于 GGUF 量化）"
    )
    parser.add_argument(
        "--from",
        dest="from_format",
