...
class ConfigSelector:
    ...
    @staticmethod
    def select_config(
        hardware: HardwareInfo, model_size_b: float, use_case: str = "general"
    ) -> DeploymentConfig:
        """
        根据硬件和使用场景选择最优配置
        ...
        # 多轮对话场景
            ...
            return DeploymentConfig(
                ...
                startup_args={"tp": 1, "mem_fraction-static": 0.9, "context-length": 8192},
            )
...
class ServiceLauncher:
    ...
    def __init__(
        self, config: DeploymentConfig, model_path: str, host: str = "0.0.0.0", port: int = 8000
    ):
        """
        初始化服务启动器
    ...
    def _build_llama_cpp_command(self) -> List[str]:
        ...
        cmd = [
            ...
            str(self.port),
            "--n-gpu-layers",
            str(self.config.startup_args.get("n_gpu_layers", 0)),
            "--ctx-size",
            str(self.config.startup_args.get("n_ctx", 2048)),
            "--batch-size",
            str(self.config.startup_args.get("n_batch", 256)),
            "--threads",
            str(self.config.startup_args.get("threads", 4)),
        ]
        return cmd
    ...
    def _build_vllm_command(self) -> List[str]:
        ...
        cmd = [
            ...
            str(self.config.startup_args.get("tensor_parallel_size", 1)),
            "--gpu-memory-utilization",
            str(self.config.startup_args.get("gpu_memory_utilization", 0.9)),
            "--max-model-len",
            str(self.config.startup_args.get("max_model_len", 4096)),
            "--quantization",
            "awq",
        ]
        ...
        if "max_num_batched_tokens" in self.config.startup_args:
            cmd.extend(
                [
                    ...
                    str(self.config.startup_args["max_num_batched_tokens"]),
                ]
            )
    ...
    def _build_sglang_command(self) -> List[str]:
        ...
        cmd = [
            ...
            str(self.port),
            "--tp",
            str(self.config.startup_args.get("tp", 1)),
            "--mem-fraction-static",
            str(self.config.startup_args.get("mem_fraction_static", 0.9)),
            "--context-length",
            str(self.config.startup_args.get("context_length", 4096)),
        ]
        return cmd
...
def deploy_model(
    ...
    logger.info(
        f"硬件信息: GPU={hardware.gpu_name}, 显存={hardware.gpu_memory_gb:.1f}GB, "
...
使用示例:
  ...
  python deploy.py --model-path ./model --model-size 7.0 --use-case multi_turn
    ...
    parser.add_argument(
        "--model-size", type=float, default=7.0, help="模型大小（十亿参数，默认: 7.0）"
    )
    parser.add_argument(
        "--use-case",
        type=str,
    ...
    parser.add_argument(
        "--no-auto-start", action="store_true", help="不自动启动服务，仅显示推荐配置"
    )
